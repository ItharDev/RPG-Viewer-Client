...
public class DrawCurve : MonoBehaviour {
	...
	void AddControlObjects () {
		// Instantiate anchor/control GUI objects, where the control object is a child of the anchor object so they move together
		anchorObject = Instantiate (anchorPoint, controlLine.points2[pointIndex], Quaternion.identity) as GameObject;
		anchorObject.transform.SetParent (canvas, true);
		anchorObject.GetComponent<CurvePointControl>().objectNumber = pointIndex++;
		var controlObject = Instantiate (controlPoint, controlLine.points2[pointIndex], Quaternion.identity) as GameObject;
		controlObject.transform.SetParent (anchorObject.transform, true);
	...
	void AddPoint () {
		...
		pointIndex++;	// Skip the next anchor point, since we want the second anchor point of one curve and the first anchor point of the next curve to move together (this is handled in UpdateLine)
		controlLine.points2[pointIndex] = controlPointPos;
		// Make another control point and have it be the child of the last anchor object we made
		var controlObject = Instantiate (controlPoint, controlPointPos, Quaternion.identity) as GameObject;
		controlObject.transform.SetParent (anchorObject.transform, true);
		controlObject.GetComponent<CurvePointControl>().objectNumber = pointIndex++;
	...
	void ChangeResolution () {
		// Make UI anchor/control points match their actual position after a resolution change
		var controlPointObjects = GameObject.FindGameObjectsWithTag ("GameController");
		foreach (var obj in controlPointObjects) {
			obj.transform.position = controlLine.points2[obj.GetComponent<CurvePointControl>().objectNumber];
		}
	}
}