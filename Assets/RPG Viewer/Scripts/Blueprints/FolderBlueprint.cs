...
namespace RPG
{
    public class FolderBlueprint : MonoBehaviour
    {
        ...
        public async void EndDrag()
        {
            ...
            if (!moved && RectTransformUtility.RectangleContainsScreenPoint(FindObjectOfType<MasterPanel>().BlueprintPanel.GetComponent<RectTransform>(), Input.mousePosition) && Path.Split("/").Length != 1)
            {
                await SocketManager.Socket.EmitAsync("move-blueprint-folder", async (callback) =>
                {
                    await UniTask.SwitchToMainThread();
