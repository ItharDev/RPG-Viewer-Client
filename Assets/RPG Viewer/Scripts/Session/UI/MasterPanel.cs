    ...
    public class MasterPanel : MonoBehaviour
    {
        ...
        private void Update()
        {
            if (!loaded && SessionManager.session != null)
            {
                ...
                if (!SessionManager.IsMaster) return;

                lastOpen = BlueprintPanel;
                LoadBlueprints();
                LoadScenes();
            }
            gameObject.SetActive(SessionManager.IsMaster);
        }
        ...
        {
            if (lastOpen == BlueprintPanel || !lastOpen.activeSelf) RotateToggle();
        ...
        {
            if (lastOpen == ScenePanel || !lastOpen.activeSelf) RotateToggle();
